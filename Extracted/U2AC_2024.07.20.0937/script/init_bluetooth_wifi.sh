...
test -e /rtl8733_rootfs.tar.gz &&  mv /rtl8733_rootfs.tar.gz /tmp/
test -e /iw416_rootfs.tar.gz &&  mv /iw416_rootfs.tar.gz /tmp/
if [ "$sdioCardID" == "0xb822" ] || [ "$sdioCardID" == "0xc822" ]; then
	...
	#insmod Wi-Fi module Driver
	test -e /tmp/8733bs.ko && insmod /tmp/8733bs.ko if2name=sta0 rtw_channel_plan=0x76
	...
	echo "4358 nvram path: $bcmWiFiConfig"
