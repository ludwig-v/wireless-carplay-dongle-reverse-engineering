...
if [ "$sdioCardID" == "0xb822" ]; then
	...
	if [ "$sdioCardID" == "0x9141" ]; then
		test -e /tmp/moal.ko && insmod /tmp/moal.ko mod_para=nxp/wifi_mod_para.conf
	elif [ "$sdioCardID" == "0x9149" ]; then
		echo "Start load SD8997!!!!!!!!!"
		test -e /tmp/moal.ko && insmod /tmp/moal.ko mfg_mode=0 drv_mode=2 cal_data_cfg=none uap_name=wlan cfg80211_wext=0xf
	fi
	set_wifi_mac
	#attach bluetooth module
	test -e /tmp/hci_uart.ko && insmod /tmp/hci_uart.ko
