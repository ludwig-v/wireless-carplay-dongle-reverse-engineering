...
if [ -d /mnt/UPAN/CARLIFE ]; then
	cp /mnt/UPAN/CARLIFE/libcarlifebox.so /tmp/lib/ 
	test -e /mnt/UPAN/CARLIFE/libcarlifebox.so && (cp /mnt/UPAN/CARLIFE/libcarlifebox.so /usr/lib/ || rm /usr/lib/libcarlifebox.so;killall fakeCarLifeDevice)
	cp /tmp/carlife.log /mnt/UPAN/CARLIFE/carlife.log
	sync
	exit 0
fi
