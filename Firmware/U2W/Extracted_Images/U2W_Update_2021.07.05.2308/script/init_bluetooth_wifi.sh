...
if [ "$sdioCardID" == "0xb822" ]; then
	...
	if [ -e /usr/sbin/brcm_patchram_plus ]; then
		...
		brcm_patchram_plus --patchram /lib/firmware/bcm/bcm4350.hcd --baudrate 1000000 --use_baudrate_for_download --no2bytes --tosleep 200000 --bd_addr $bcmBTMac /dev/ttymxc2 --enable_hci &
	fi
	#insmod Wi-Fi module Driver
