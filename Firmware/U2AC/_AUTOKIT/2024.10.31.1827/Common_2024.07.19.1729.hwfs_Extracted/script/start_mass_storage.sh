...
#!/bin/bash
#For Automatic Test
test -e /tmp/.AutomaticTestMode && exit 0
...
#For set oem config
test -e /mnt/UPAN/box_oem_config.txt && cp /mnt/UPAN/box_oem_config.txt /etc/riddle_special.conf && riddleBoxCfg --specialConfig && riddleBoxCfg --upConfig && echo 2 >  /tmp/update_status && exit 0
#For ICCOA Debug
