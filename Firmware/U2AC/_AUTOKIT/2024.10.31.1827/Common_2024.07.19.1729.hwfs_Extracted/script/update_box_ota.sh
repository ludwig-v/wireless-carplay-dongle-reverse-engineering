...
doUpdate() {
	...
	if [ $result -ne 0 ] ; then
		...
		if [ $1 = "img" ]; then
			test -e /etc/module_version/ && rm -r /etc/module_version/
		fi
...
#To be compatible with 'img' and 'zip' files when automatic upgrades
if [ -e /data/update/is_module_update ];then 
	mv /tmp/*_Update.img /tmp/`cat /etc/box_product_type`_Update.zip
	rm /data/update/is_module_update
fi
