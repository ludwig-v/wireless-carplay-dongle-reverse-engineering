...
#update when box_version changed
#test -e /tmp/update/tmp/box_version && cp /tmp/update/tmp/box_version /etc/
...
if [ -e /tmp/update/etc/riddle_default.conf ]; then
	...
	result=$?
	if [ "$result" -ne 0 ]; then
		mv /tmp/update/etc/riddle_default.conf /etc/
		...
		echo "special config: `cat /etc/riddle_special.conf`"
		echo "default config: `cat /etc/riddle_default.conf`"
	fi
fi	
...
if [ "$sdioCardID" == "0xb822" ]; then
	...
	if [ -e /tmp/update/rtl8822cs_rootfs.tar.gz ]; then
		...
		rm -f /tmp/update/etc/wpa_supplicant.conf
	fi
	...
	if [ -e /tmp/update/iw416_rootfs.tar.gz ]; then
		tar -xvf /tmp/update/iw416_rootfs.tar.gz -C /tmp/update/
	else
		rm -f /tmp/update/etc/wpa_supplicant.conf
	fi
fi
...
rm -f /tmp/update/iw416_rootfs.tar.gz
