...
autoconf_fail:
	...
	return usb_assign_descriptors(f, fs_source_sink_descs,
			hs_source_sink_descs, NULL, NULL);
...
DECLARE_USB_FUNCTION(PTP, source_sink_alloc_inst,
		source_sink_alloc_func);

static int __init ptp_modinit(void)
{
	return usb_function_register(&PTPusb_func);
}
static void __exit ptp_modexit(void)
{
	usb_function_unregister(&PTPusb_func);
}
module_init(ptp_modinit);
module_exit(ptp_modexit);
