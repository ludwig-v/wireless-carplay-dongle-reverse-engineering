...
/*
 * Linux 3.14 routed an unhandled WEXT private ioctl to ndo_do_ioctl().
 * That fallback was removed before Linux 5.10.  The vendor hostapd uses
 * SIOCIWFIRSTPRIV + 0x1c directly, so expose the existing hostapd ioctl
 * implementation through the WEXT private-handler table as well.
 */
static int rtw_wx_hostapd_ioctl(struct net_device *dev,
			       struct iw_request_info *info,
			       union iwreq_data *wrqu, char *extra)
{
	return rtw_hostapd_ioctl(dev, &wrqu->data);
}
#endif
...
#endif
#ifdef CONFIG_AP_MODE
	rtw_wx_hostapd_ioctl,				/* 0x1C hostapd */
#else
	NULL,
#endif
	rtw_test,						/* 0x1D */
